...
export default function LevelTwo({ navigation }) {
  ...
  <Text style={styles.contentText}>
			Então, você achava que sabia contar? Bem, aqui está uma nova forma de fazer isso! Sabia que os computadores
			utilizam apenas zeros e uns?
  </Text>,
  <Text style={styles.contentText}>
			Tudo o que você vê ou ouve no computador - palavras, imagens, números, filmes e até mesmo o som - são
			armazenados usando apenas estes dois numerais! Estas atividades ensinarão como enviar mensagens secretas aos
			seus amigos usando exatamente o mesmo método que um computador.
  </Text>,
  <View>
    <Text style={[ styles.contentText, { marginTop: 10 } ]}>Presione o cartão para vira-lo</Text>
    ...
    <Text style={styles.contentText}>As cartas acima estão representando o número 9.</Text>
  </View>,
  ...
  <View style={styles.descriptive}>
    <KeyboardAvoidingView style={styles.subDescriptive} behavior="padding" enabled>
      <CustomTextInput
  ...
  <ScrollView>
    <View style={styles.container}>
      <HeaderOfExcercises style={styles.header} title="Nível 2 - Números Binários" navigation={navigation} />
      <CustomBackgruond content={viewOfContent} style={styles.info} />
      {changeGroupButton()}
      <ProgressBar style={styles.progressBar} color={colors.colorSucess} progress={progrees} />
    </View>
  </ScrollView>
	);
}
